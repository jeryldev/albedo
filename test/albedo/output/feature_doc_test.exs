defmodule Albedo.Output.FeatureDocTest do
  ...
  describe "header generation" do
    test "includes task name" do
      header = generate_header(%{task: "Add feature X", session_dir: "/path/to/session"})
      assert header =~ "# Feature: Add feature X"
    end

    test "includes Albedo version" do
      header = generate_header(%{task: "Task", session_dir: "/path"})
    ...
    test "includes session directory" do
      header = generate_header(%{task: "Task", session_dir: "/path/to/my-session"})
      assert header =~ "/path/to/my-session"
    end
  end
